...
Future<Widget> generateAbilityScores(Character character) async {
  ...
  Proficiency classDC = character.pathClass.proficiencies.firstWhere((item) => item.proficiency_type == 'CDC', orElse: () => Proficiency(name: 'FAILURE'));
...
Widget generateGenericInfo(Character character){
  return buildGenericInformationRow(
    ...
    background: '',
    nameClass: character.pathClass.name ?? '',
...
Future<Widget> generateHPPerception(Character character) async {
  List<Proficiency> proficiencies = character.pathClass.proficiencies;
  ...
  return buildHPPerception(
    hpMax: (character.ancestry.hit_points + character.pathClass.hit_points + character.constitution.modifier).toString(),
    hpTemp: '',
    hpCurrent: '',
    hpDying: '',
    hpWounded: '',
...
Future<Widget> generateFeatsAndFeatures(Character character) async{
  ...
  List<Feat> itemlist = character.pathClass.features;
